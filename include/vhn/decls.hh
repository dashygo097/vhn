...
#define ELEMENTWISE_TB_DEF(ELEMENTWISE_NAME)                                   \
  ...
  private:                                                                     \
    using ELEMENTWISE_NAME##DUT =                                              \
        vhn::ELEMENTWISE_NAME<DType, N, Config, OPT_LEVEL>;                    \
    using ELEMENTWISE_NAME##Ref =                                              \
        vhn::ELEMENTWISE_NAME<float, N, void, OPT_NONE>;                       \
...
#define REDUCE_TB_DEF(REDUCE_NAME)                                             \
  ...
  template <typename DType, const int N, typename Config = void,               \
            OptLevel OPT_LEVEL = OPT_NONE>                                     \
  class Reduce##REDUCE_NAME##Testbench {                                       \
  public:                                                                      \
    Reduce##REDUCE_NAME##Testbench() = default;                                \
    ~Reduce##REDUCE_NAME##Testbench() = default;                               \
    void test_random_case(const std::string &case_REDUCE_NAME) {               \
      _generator.generate_random_input(_input_ref);                            \
                                                                               \
      _convert_input(_input_ref, _input_dut);                                  \
                                                                               \
      Reduce##REDUCE_NAME##DUT::forward(_output_dut, _input_dut);              \
      Reduce##REDUCE_NAME##Ref::forward(_output_ref, _input_ref);              \
                                                                               \
      float output_dut_float;                                                  \
      _convert_output(_output_dut, output_dut_float);                          \
                                                                               \
      auto result =                                                            \
          ResultComparator::compare(&output_dut_float, &_output_ref, 1);       \
      ResultComparator::print_result(result, case_REDUCE_NAME);                \
    }                                                                          \
    void test_identity_case() {                                                \
      _generator.generate_random_input(_input_ref);                            \
                                                                               \
    ...
    void run_all_tests() {                                                     \
      std::cout << "\n########################################" << std::endl;  \
      std::cout << "Testing Reduce" #REDUCE_NAME "Layer" << std::endl;         \
      std::cout << "N: " << N << std::endl;                                    \
      std::cout << "OPT_STATUS: " << OPT_LEVEL << std::endl                    \
                << "DType: " << typeid(DType).name() << std::endl;             \
      std::cout << "########################################" << std::endl;    \
      ...
      test_ones_case();                                                        \
                                                                               ...
                                                                               \
  private:                                                                     \
    using Reduce##REDUCE_NAME##DUT =                                           \
        vhn::Reduce##REDUCE_NAME<DType, N, Config, OPT_LEVEL>;                 \
    using Reduce##REDUCE_NAME##Ref =                                           \
        vhn::Reduce##REDUCE_NAME<float, N, void, OPT_NONE>;                    \
    Reduce##REDUCE_NAME##TestCase<N> _generator;                               \
                                                                               \
    DType _input_dut[N];                                                       \
    DType _output_dut;                                                         \
                                                                               \
    ...
    void _convert_input(const float src[N], DType dst[N]) {                    \
      for (int i = 0; i < N; i++) {                                            \
        dst[i] = static_cast<DType>(src[i]);                                   \
                                                                               ...
                                                                               \
    void _convert_output(const DType &src, float &dst) {                       \
