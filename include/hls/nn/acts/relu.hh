...
namespace hls_tb {
template <const int N> class ReLUTestCase {
public:
  ReLUTestCase(unsigned seed = 42) : _seed(seed), _dist(-1.0f, 1.0f) {}
...
public:
  ...
  void test_random_case(const std::string &case_name) {
    ...
    ReLUDUT::forward(_output_dut, _input_dut);
    ReLURef::forward(_output_ref, _input_ref);
  ...
  void run_all_tests() {
    std::cout << "\n########################################" << std::endl;
    std::cout << "Testing ReLU Layer" << std::endl;
...
private:
  using ReLUDUT = hls_nn::ReLU<DType, N, Config, OPT_LEVEL>;
  using ReLURef = hls_nn::ReLU<float, N, void, OPT_NONE>;
  ReLUTestCase<N> _generator;
